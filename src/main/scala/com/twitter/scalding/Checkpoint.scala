...
object Checkpoint {
   ...
   *
   * Implicit parameters:
   *   args:    provided by com.twitter.scalding.Job
  ...
  def apply[A](checkpointName : String, resultFields : Fields)
              (flow : => Pipe)
              (implicit args : Args, mode : Mode, flowDef : FlowDef,
    ...
    if (filename.isDefined && hasInput(checkpointName, filename.get)) {
      ...
      LOG.info("Checkpoint \"" + checkpointName + "\": reading input from \"" + filename.get + "\"")
      getSource(checkpointName, filename.get)
        .read
      ...
      if (filename.isDefined) {
        LOG.info("Checkpoint \"" + checkpointName + "\": writing output to \"" +
                 filename.get + "\"")
        pipe.write(getSource(checkpointName, filename.get))
      } else {
        pipe
      }
    }
  ...
  // Returns a source for the checkpoint.  The format of the source is
  // determined by the flag --checkpoint.format, and defaults to SequenceFile.
  private def getSource(checkpointName : String, filename : String)
                       (implicit args : Args, mode : Mode) : Source = {
    ...
    format match {
      ...
      case _ => error("Invalid value for --checkpoint.format: " + format)
    }
  }
  ...
  private def hasInput(checkpointName : String, filename : String)
    ...
    !CheckpointArg(checkpointName, "clobber").value.isDefined && mode.fileExists(filename)
  }
}
