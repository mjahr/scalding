...
class CheckpointTestJob(args : Args) extends Job(args) {
...
class CheckpointTest extends Specification with TupleConversions {
  "A CheckpointTestJob" should {
    ...
    "run without checkpoints" in runTest {
      JobTest("com.twitter.scalding.CheckpointTestJob")
